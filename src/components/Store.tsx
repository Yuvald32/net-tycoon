...
import { findNextFreePosition } from '../utils/grid'

const DEVICE_TYPES: Set<string> = new Set(['PC', 'Switch', 'Router'])

const ALL_ITEMS: { name: ShopItem; icon: string }[] = [
  { name: 'PC', icon: '🖥️' },
  { name: 'Switch', icon: '🔀' },
  { name: 'Router', icon: '📡' },
  { name: 'Cat6 Cable', icon: '🔌' },
]

function createDevice(type: DeviceType, existingDevices: Device[]): Device {
  ...
  return {
    ...
    position: findNextFreePosition(existingDevices),
  }
}
...
export function Store() {
  ...
  const availableTools = level?.availableTools ?? []

  const deviceItems = ALL_ITEMS.filter(
    (item) => DEVICE_TYPES.has(item.name) && availableTools.includes(item.name)
  )
  const hasCable = availableTools.includes('Cat6 Cable')
  ...
  function handleBuyDevice(item: ShopItem) {
    ...
    if (DEVICE_TYPES.has(item)) {
      addDevice(createDevice(item as DeviceType, devices))
    }
  }
